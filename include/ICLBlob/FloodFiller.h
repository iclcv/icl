...
**                                                                 **
** File   : include/ICLBlob/FloodFiller.h                          **
** Module : ICLBlob                                                **
...
namespace icl{
  ...
  */
  class FloodFiller{
    ...
    std::vector<Point> futurePoints;
    
    /// internal utility method
    Rect prepare(const Size &imageSize, const Point &seed);

    public:

    /// result structure, returned by the 'apply' methods
    struct Result{
      std::vector<Point> pixels; /// list of filled pixels
      Img8u ffLUT;               /// binary mask with filled pixels
    } result;
    ...
    const Result &apply(const ImgBase *image, const Point &seed, double referenceValue, double threshold);
    ...
    const Result &applyColor(const ImgBase *image, const Point &seed, 
    ...
    inline const Result &applyGeneric(const Img<T> &image, const Point &seed, Criterion crit){
      Rect r = prepare(image.getSize(),seed);
      
      Point *n = futurePoints.data(); // next point to seed from
      Point *e = n;                   // end of stored seed points
      
      *e++ = seed;
      
      const Channel<T> im = image[0];
      Channel8u ff = result.ffLUT[0];
      ...
      while(n != e){
        ...
        ++n;
        if(crit(im(x,y))){
          result.pixels.push_back(Point(x,y));
...
#undef ICL_DYNAMIC_FLOOD_FILLER_P
    ...
    inline const Result &applyColorGeneric(const Img<T> &image, const Point &seed, Criterion3Channels crit){
      Rect r = prepare(image.getSize(),seed);
      ...
      *e++ = seed;
      ...
      const Channel<T> im0 = image[0];
      const Channel<T> im1 = image[1];
      const Channel<T> im2 = image[2];
...
#undef ICL_DYNAMIC_FLOOD_FILLER_P
  ...
  struct FloodFiller::ReferenceColorCriterion<icl8u>{
    ...
    inline bool operator()( const icl8u &r, const icl8u &g, const icl8u &b) const{
      return sqr(int(r)-refcol[0]) + sqr(int(g)-refcol[1]) + sqr(int(b)-refcol[2]) < maxSquaredEuklDist;
    }
  };
  /** \endcond **/
