...
**                                                                 **
** File   : include/ICLUtils/RansacFitter.h                        **
** Module : ICLUtils                                               **
...
namespace icl{
  ...
  /** The RansacFitter provides a generic framework, for RANSAC based model fitting.
      ...
      as an example application: ICL/ICLUtils/examples/ransac-test.cpp
  ...
  */
  ...
  class RansacFitter{
    ...
    struct Result{
      icl64f error;
      Model model;
      std::vector<DataPoint> consensusSet;
      int iterationCount;
    ...
    const Result &fit(const DataSet &allPoints){
      ...
      for(i=0;i<m_iterations;++i){
        ...
        for(int j=0;j<(int)allPoints.size();++j){
          ...
          if(m_err(model, allPoints[j]) < m_minModelDistance){
