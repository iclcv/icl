...
namespace icl{
  ...
  OSDNavBar::OSDNavBar(int id, QRect r,ImageWidget* poIW , OSDWidget *poParent,svec vecNames, svec vecShortNames, ivec vecIDs, int selectedIndex):
    ...
    for(int i=0;i<n;i++){
      QRect oCurrRect(curr_x,s_iMargin+y(),m_iSmallW-1,h); 
  ...
  void OSDNavBar::selectIndex(int index){
    ...
    for(int i=0;i<n;i++){
      ...
      if(i==index){
        ...
        b->setRect(QRect(curr_x,s_iMargin+y(),iWideWidth-1,h));
        curr_x += iWideWidth+s_iGap;
        b->setText(m_vecNames[i]);
        b->setToggled(1);
      }else{
        m_vecChilds[i]->setRect(QRect(curr_x,s_iMargin+y(),m_iSmallW-1,h));
        curr_x += m_iSmallW+s_iGap;
