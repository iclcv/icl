...
namespace icl {
  ...
  /** This function provides a common interface to instantiate images of 
    ...
    **/
    ICL_CORE_API ImgBase *imgNew(depth d = depth8u, const ImgParams &params = ImgParams::null);
    ...
    **/
    ICL_CORE_API ImgBase *ensureDepth(ImgBase **ppoImage, depth eDepth);
  
    /// ensures that an image has given depth and parameters \ingroup IMAGE
    ICL_CORE_API ImgBase *ensureCompatible(ImgBase **dst, depth d, const ImgParams &params);
    ...
    */
    ICL_CORE_API ImgBase *ensureCompatible(ImgBase **dst, depth d, const utils::Size &size, int channels, format fmt, const utils::Rect &roi = utils::Rect::null);
    ...
    **/
    ICL_CORE_API ImgBase *ensureCompatible(ImgBase **dst, const ImgBase *src);
    ...
    **/
    ICL_CORE_API int getChannelsOfFormat(format fmt);
    ...
    ICL_CORE_API std::ostream &operator<<(std::ostream &s, const format &f);
    
    /// puts a string representation of depth into the given stream
    ICL_CORE_API std::ostream &operator<<(std::ostream &s, const depth &d);
  
    /// puts a string representation of format into the given stream
    ICL_CORE_API std::istream &operator>>(std::istream &s, format &f);
    
    /// puts a string representation of depth into the given stream
    ICL_CORE_API std::istream &operator>>(std::istream &s, depth &d);
  ...
  #undef ICL_INSTANTIATE_DEPTH
    ...
    ICL_CORE_API unsigned int getSizeOf(depth eDepth);
    ...
    /// from icl8u functions
    template<> ICL_CORE_API void convert<icl8u,icl32f>(const icl8u *poSrcStart,const icl8u *poSrcEnd, icl32f *poDst);

    /// from icl16s functions
    template<> ICL_CORE_API void convert<icl16s,icl32s>(const icl16s *poSrcStart,const icl16s *poSrcEnd, icl32s *poDst);
    template<> ICL_CORE_API void convert<icl16s,icl32f>(const icl16s *poSrcStart,const icl16s *poSrcEnd, icl32f *poDst);
    template<> ICL_CORE_API void convert<icl16s,icl64f>(const icl16s *poSrcStart,const icl16s *poSrcEnd, icl64f *poDst);
    
    // from icl32s functions
    template<> ICL_CORE_API void convert<icl32s,icl16s>(const icl32s *poSrcStart,const icl32s *poSrcEnd, icl16s *poDst);
    template<> ICL_CORE_API void convert<icl32s,icl32f>(const icl32s *poSrcStart,const icl32s *poSrcEnd, icl32f *poDst);
    template<> ICL_CORE_API void convert<icl32s,icl64f>(const icl32s *poSrcStart,const icl32s *poSrcEnd, icl64f *poDst);
  
    // from icl32f functions
    template <> ICL_CORE_API void convert<icl32f,icl8u>(const icl32f *poSrcStart, const icl32f *poSrcEnd, icl8u *poDst);
    template <> ICL_CORE_API void convert<icl32f,icl16s>(const icl32f *poSrcStart, const icl32f *poSrcEnd, icl16s *poDst);
    template <> ICL_CORE_API void convert<icl32f,icl32s>(const icl32f *poSrcStart, const icl32f *poSrcEnd, icl32s *poDst);
    template <> ICL_CORE_API void convert<icl32f,icl64f>(const icl32f *poSrcStart, const icl32f *poSrcEnd, icl64f *poDst);
  
    // from icl64f functions 
    template<> ICL_CORE_API void convert<icl64f,icl32f>(const icl64f *poSrcStart,const icl64f *poSrcEnd, icl32f *poDst);
    template <> ICL_CORE_API void convert<icl64f,icl32s>(const icl64f *poSrcStart,const icl64f *poSrcEnd, icl32s *poDst);
    ...
    */
    ICL_CORE_API std::vector<double> mean(const ImgBase *poImg, int iChannel = -1, bool roiOnly = false);
    ...
    */
    ICL_CORE_API std::vector<double> variance(const ImgBase *poImg, const std::vector<double> &mean, bool empiricMean = true, int iChannel = -1, bool roiOnly = false);
	...
	@param roiOnly 
        @return The variance value form the vector
        */
    ICL_CORE_API std::vector<double> variance(const ImgBase *poImg, int iChannel = -1, bool roiOnly = false);
    ...
    /** @param poImage input image
        ...
        @param roiOnly
        */
    ICL_CORE_API std::vector<double> stdDeviation(const ImgBase *poImage, int iChannel = -1, bool roiOnly = false);
	...
	@param roiOnly
    */
    ICL_CORE_API std::vector<double> stdDeviation(const ImgBase *poImage, const std::vector<double> mean, bool empiricMean = true, int iChannel = -1, bool roiOnly = false);
    ...
    */
    ICL_CORE_API std::vector< std::pair<double, double> > meanAndStdDev(const ImgBase *image, int iChannel = -1, bool roiOnly = false);
    
    
    /// computes the color histogramm of given image channel                               
    ICL_CORE_API std::vector<int> channelHisto(const ImgBase *image, int channel, int levels = 256, bool roiOnly = false);
    
    /// computes the color histogramm of given image
    ICL_CORE_API std::vector<std::vector<int> > hist(const ImgBase *image, int levels = 256, bool roiOnly = false);
