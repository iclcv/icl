...
namespace icl{
  ...
  inline void LineSampler::bresenham_templ(int x0, int x1, int y0, int y1, int minX, int maxX, int minY, int maxY, Point *p){
    Point *buf = p;
    int deltax = x1 - x0;
  ...
  template<bool steep, bool steep2, int ystep>
  inline void LineSampler::bresenham_templ_2(int x0, int x1, int y0, int y1, Point *p){
    Point *buf = p;
    int deltax = x1 - x0;
    int deltay = std::abs(y1 - y0);
    int error = 0;
  ...
  void LineSampler::bresenham(int x0, int x1, int y0, int y1, int minX, int maxX, int minY, int maxY, Point *buf){
  ...
  void LineSampler::bresenham(int x0, int x1, int y0, int y1, Point *buf){
    bool steep = std::abs(y1 - y0) > std::abs(x1 - x0);
    if(steep){
      std::swap(x0, y0);
      std::swap(x1, y1);
