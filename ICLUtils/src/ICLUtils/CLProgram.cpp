	...
	namespace utils {
		...
		struct CLProgram::Impl {
			...
			static void listDeviceInfos(cl::Device & device) {
				...
				listInfo("CL_DEVICE_MAX_COMPUTE_UNITS", toString(uintValue));

				device.getInfo(CL_DEVICE_MAX_WORK_GROUP_SIZE, &sizeValue);
				listInfo("CL_DEVICE_MAX_WORK_GROUP_SIZE", toString(sizeValue));
				device.getInfo(CL_DEVICE_MAX_MEM_ALLOC_SIZE, &ulongValue);
				listInfo("CL_DEVICE_MAX_MEM_ALLOC_SIZE", toString(ulongValue));

				device.getInfo(CL_DEVICE_GLOBAL_MEM_SIZE, &ulongValue);
				listInfo("CL_DEVICE_GLOBAL_MEM_SIZE", toString(ulongValue));

				device.getInfo(CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE, &ulongValue);
				listInfo("CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE", toString(ulongValue));

				device.getInfo(CL_DEVICE_MAX_CONSTANT_ARGS, &uintValue);
				listInfo("CL_DEVICE_MAX_CONSTANT_ARGS", toString(uintValue));
				...
				listInfo("CL_DEVICE_LOCAL_MEM_SIZE", toString(ulongValue));
