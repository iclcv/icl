...
CalibrationObject *obj = 0;

static const int VIEW_CAM = 0, CALIB_CAM = 1;

void show_hide_scene(){  sceneGUI.switchVisibility(); }

DragRectangleMouseHandler dragRect;
...
void init(){
  ...
  obj = new CalibrationObject(new CalibrationGrid(*pa("-c")),"co");
  ...
  /// scene GUI (not shown on startup!)
  sceneGUI << (GUI("hbox") 
           ...
           << (GUI("tab(real camera,free camera)[@size=32x24]")
               ...
               << "draw3D[@handle=scene@minsize=16x12]");
  sceneGUI.create();
  ...
  (*sceneGUI.getValue<DrawHandle3D>("scene"))->setImageInfoIndicatorEnabled(false);
  (*sceneGUI.getValue<DrawHandle3D>("calibScene"))->setImageInfoIndicatorEnabled(false);
  sceneGUI["scene"].install(scene.getMouseHandler(0));
  sceneGUI["calibScene"].install(scene.getMouseHandler(1));

  
  /// main GUI
  gui << "draw3D[@minsize=16x12@label=main view@handle=mainview]"
      << ( GUI("vbox")
           <<  (GUI("hbox") 
           ...
           <<  (GUI("hbox") 
                << "togglebutton(paused,!grabbing)[@out=grab]"
                << "camcfg()"
...
void run(){
  ...
  if(grab || !image){
    image = SmartPtr<const ImgBase>(grabber.grab()->shallowCopy(dragRect.getRect()));
  }
  ...
  if(::gui["2D"]){
    obj->visualize2D(**mainview,false);
  }else{
    mainview = image.get();
    mainview->lock();
    mainview->reset();
  ...
  if(::gui["mouseROI"]){
    dragRect.m_outer[3]=0;
    dragRect.m_outerWhileDrag[3]=0;
  }else{
    dragRect.m_outer[3]=150;
    dragRect.m_outerWhileDrag[3]=50;
  }
  dragRect.visualize(**mainview);
