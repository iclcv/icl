...
PointCloudCreatorCL::PointCloudCreatorCL(Size size, const Array2D<Vec> &dirs) {
	...
	if(clReady==true) { //only if CL context is available
		try {
			...
			dirsBuffer = program.createBuffer("rw", size.width*size.height * 4 * sizeof(float), dirsArray);
...
void PointCloudCreatorCL::createRGB(bool NEEDS_RAW_TO_MM_MAPPING,
	...
	try {
		...
		depthValuesBuffer = program.createBuffer("r", DEPTH_DIM * sizeof(float), depthValuesArray);
		rInArray = (unsigned char*)rgbIn->begin(0);
		rInBuffer = program.createBuffer("r", DEPTH_DIM * sizeof(unsigned char), rInArray);
		gInArray = (unsigned char*)rgbIn->begin(1);
		gInBuffer = program.createBuffer("r", DEPTH_DIM * sizeof(unsigned char), gInArray);
		bInArray = (unsigned char*)rgbIn->begin(2);
		bInBuffer = program.createBuffer("r", DEPTH_DIM * sizeof(unsigned char), bInArray);
