...
I.e. ICLQuick is designed for float-images only, which underlines it's prototyping character. To facilitate usage if ICLQuick's function in combination with images of other types, conversion functions are provided (see subsection \ref{subsubsec:quick-converters}). However the choice of the float image type was obvious as floats are a good compromise between performance and flexibility (see section \ref{sec:image-value-domain} for more details).

\subsection{ICLQuick Functions}
The function set of the ICLQuick package can be divided into the following subsets:

\begin{itemize}
\item \textbf{Creator Functions} [see \ref{subsubsec:quick-creators}]
\item \textbf{Converter Functions} [see \ref{subsubsec:quick-converters}]
\item \textbf{Filtering Functions} [see \ref{subsubsec:quick-filters}]
\item \textbf{Output Functions} [see \ref{subsubsec:quick-output}]
\item \textbf{Arithmetical Operators} [see \ref{subsubsec:quick-arith-operators}]
\item \textbf{Arithmetical Functions} [see \ref{subsubsec:quick-arith-functions}]
\item \textbf{Logical Operators} [see \ref{subsubsec:quick-logical-operators}]
\item \textbf{Image Concatenation Operators} [see \ref{subsubsec:quick-concatenation}]
\item \textbf{Data/ROI Assignment Functions} [see \ref{subsubsec:quick-roi-assignment}]
\item \textbf{Drawing Functions} [see \ref{subsubsec:quick-drawing}]
\item \textbf{tic/toc} [see \ref{subsubsec:quick-tic-toc}] 
\item \textbf{Region Detection/Analysis} [see \ref{subsubsec:quick-regions}]  
\end{itemize}

\subsubsection{Creator Functions\label{subsubsec::quick-creators}}
TODO


\subsubsection{Conversion Functions\label{subsubsec:quick-cvt}}
TODO
