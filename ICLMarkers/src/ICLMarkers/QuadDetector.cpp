...
const std::vector<TiltedQuad> &QuadDetector::detect(const ImgBase *image) {
    ...
    for (unsigned int i = 0; i < rs.size(); ++i) {
        ...
        {
            corners = data->css.detectCorners(boundary);
        }
        std::vector<Point32f> cornersCopy = corners;

        allCorners.push_back(corners);
        if (cornersCopy.size() > 4) {
            cornersCopy = data->removeObstacleCorners(cornersCopy,
                    getPropertyValue("min-rating"),
                    getPropertyValue("intersection heuristic"),
                    getPropertyValue("perpendicular heuristic"),
                    getPropertyValue("mirror heuristic"));
        }
        if (cornersCopy.size() == 4) {
            if (optEdges) {
                try {
//					optimize_edges(cornersCopy, boundary); // todo optimize !!
                    data->quads.push_back(
                            TiltedQuad(cornersCopy.data(), rs[i]));
                } catch (int code) {
                    (void) code;
                }
            } else {
...
std::vector<std::vector<icl::utils::Point32f> > QuadDetector::getLongestCorners() {
    return data->longestCorners;
}

std::vector<std::vector<icl::utils::Point32f> > QuadDetector::getPerpCorners() {
    return data->perpCorners;
}

std::vector<std::vector<icl::utils::Point32f> > QuadDetector::getMirrorCorners() {
    return data->mirrorCorners;
}

std::vector<std::vector<icl::utils::Point32f> > QuadDetector::getInterCorners() {
    return data->interCorners;
}

std::vector<std::vector<icl::utils::Point32f> > QuadDetector::getSecLongestCorners() {
