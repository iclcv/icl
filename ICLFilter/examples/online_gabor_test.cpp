...
private:
  GUI gui;
  inline bool equal(const float *a, const float *b, unsigned int n){
...
public:
  ...
  virtual void run(){
    ...
    Grabber *grabber = new PWCGrabber(Size(640,480),24,0);
    ...
    while(1){
      ...
      if(!equal(params,saveParams,5) || size != saveSize || !g){
        if(g) delete g;
