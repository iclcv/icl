...
**                                                                 **
** File   : ICLFilter/test/test-template.cpp                       **
** Module : ICLFilter                                              **
...
TEST(UnaryOp, LogicalOp) {
  icl8u a[] = {0,128,255};
  Img8u src(Size(4,1),1,std::vector<icl8u*>(1,a));
  
  UnaryLogicalOp op(UnaryLogicalOp::andOp, 0xF);
  
  const icl8u *b = op.apply(&src)->as8u()->begin(0);
  
  for(int i=0;i<4;++i){
    ASSERT_EQ(a[i]&0xF,b[i]) << "unary and with 0xF did not work";
  }
}
