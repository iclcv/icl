...
**                                                                 **
** File   : ICLFilter/src/ICLFilter/BilateralFilterOp_Cpp.cpp      **
** Module : ICLFilter                                              **
...
namespace {
  ...
  // C++ fallback: brute-force O(r^2) per pixel bilateral filter.
  //
  // Possible optimizations (not yet implemented):
  // - Range weight LUT: for 8u, only 256 possible |diff| values — precompute
  //   exp(-d^2/(2*sigma_r^2)) into a 256-entry table instead of calling exp() per neighbor.
  ...
  // Layout: spatialLUT[(dy+r) * side + (dx+r)] = exp(-dist^2/(2*sigma_s^2))
