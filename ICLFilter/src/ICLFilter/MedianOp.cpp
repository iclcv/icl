...
namespace icl {
  namespace filter{
    ...
    MedianOp::MedianOp(const Size &maskSize) : NeighborhoodOp(adaptSize(maskSize)) {
      initDispatching("MedianOp");
      auto &fixed = addSelector<MedianFixedSig>("fixed");
      fixed.add(Backend::Cpp, cpp_median_fixed);
      auto &generic = addSelector<MedianGenericSig>("generic");
      generic.add(Backend::Cpp, cpp_median_generic);
    }
    ...
    void MedianOp::apply(const core::Image &src, core::Image &dst) {
      ...
      if (ms == Size(3,3) || ms == Size(5,5)) {
        getSelector<MedianFixedSig>("fixed").resolve(src)->apply(
          src, dst, ms.width, getROIOffset());
      } else {
        getSelector<MedianGenericSig>("generic").resolve(src)->apply(
