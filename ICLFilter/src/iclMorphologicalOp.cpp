...
namespace icl {
  ...
  void MorphologicalOp::checkMorphAdvState8u(const Size roiSize){
    if (m_bHas_changedAdv){
      deleteMorphStates();
      ippiMorphAdvInitAlloc_8u_C1R(&m_pAdvState8u, roiSize, m_pcMask, m_oMaskSize, m_oAnchor);
    ...
    if (m_bHas_changed){
      ...
      ippiMorphologyInitAlloc_8u_C1R(roiSize.width, m_pcMask, m_oMaskSize, m_oAnchor,&m_pState8u);
  ...
  void MorphologicalOp::checkMorphAdvState32f(const Size roiSize){
    ...
    if (m_bHas_changed){
      ...
      ippiMorphologyInitAlloc_32f_C1R(roiSize.width, m_pcMask, m_oMaskSize, m_oAnchor,&m_pState32f);
  ...
  void MorphologicalOp::ippiMorphologicalCall (const Img<T> *src, Img<T> *dst) {
    for(int c=0; c < src->getChannels(); c++) {
        ippiFunc(src->getROIData (c, this->m_oROIOffset),
                 src->getLineStep(),
                 dst->getROIData (c),
                 dst->getLineStep(),
  ...
  void MorphologicalOp::ippiMorphologicalCall3x3 (const Img<T> *src, Img<T> *dst) {
    for(int c=0; c < src->getChannels(); c++) {
        ippiFunc(src->getROIData (c, this->m_oROIOffset),
                 src->getLineStep(),
                 dst->getROIData (c),
  ...
  void MorphologicalOp::ippiMorphologicalBorderReplicateCall (const Img<T> *src, Img<T> *dst,IppiMorphState* state) {
    for(int c=0; c < src->getChannels(); c++) {
        ippiFunc(src->getROIData (c, this->m_oROIOffset),
                 src->getLineStep(),
                 dst->getROIData (c),
  ...
  void MorphologicalOp::ippiMorphologicalBorderCall (const Img<T> *src, Img<T> *dst, IppiMorphAdvState* advState) {
    for(int c=0; c < src->getChannels(); c++) {
        ippiFunc(src->getROIData (c, this->m_oROIOffset),
                 src->getLineStep(),
                 dst->getROIData (c),
