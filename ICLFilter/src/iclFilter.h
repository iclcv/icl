...
/**
    ...
    of filters is are such filters that get exactly two input images and that have only a single output
    image (e.g. arithmetical/logical per-pixel image operations or image comparison filters).\n
    ...
    obviate further misunderstandings, we call this filter groups <b>Unary Operations</b> and 
    <b>Binary-Operations</b> - or short UnaryOp and BinaryOp. \n
    Each of this groups is represented by a C++-class with that name that is inherited by all implemented
    filters that belong to this group (from now on <em>Ops</em>(for operations).\n
