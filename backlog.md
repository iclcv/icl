...
- [ ] **camera-calibration REDESIGN** (multi-session, ACTIVE) — plan + findings:
  ...
  - [ ] **RESEARCH: better checkerboard detectors** — detection bounds calibration accuracy; survey SOTA (findChessboardCornersSB/ROCHADE, libcbdetect/Geiger growth, DL corner detectors) vs our ChESS+growth; wrap the best as another `CheckerboardDetector` backend / `CornerSeed` provider if it beats native.
