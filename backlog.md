...
- [ ] **camera-calibration REDESIGN** (multi-session, ACTIVE) — plan + findings:
  ...
  - [ ] **comparison harness** — drive each `CalibrationTarget` / `CheckerboardDetector` backend through `Camera::calibrate_*` over a distance/noise/distortion/viewpoint sweep → error-vs-truth tables (2D-checkerboard-vs-3D-marker, native-vs-opencv). THE numbers.
  ...
  - [ ] **DEEP RESEARCH — robust checkerboard grid ASSOCIATION** (NEXT). Greedy growth + edge-evidence bootstrap falls into the diagonal trap under steep oblique views. Survey SOTA (libcbdetect/Geiger energy growth, findChessboardCornersSB/ROCHADE, Delaunay/graph topology + orientation cue, global homography-ICP/Hungarian) → pick a principled foreshortening-robust association; validate against real lab frames (`save frame` tool). Subsumes the old "better detectors" item.
