...
- [x] **Soft-shadow softness — DONE (S103).** `LightNode::softShadowRadius` → per-light `ShadowOptions::shadowBulbRadius` (× `softShadowScale`, default 1.5); any soft light flips the View PCF→PCSS. `render.soft shadow scale` knob. Default unchanged (radius 0 = clean PCF). Verified `icl-filament-pcss-test`. NB PCSS grainy single-shot (needs TAA), smooth in the live tuner.
