...
- [ ] **camera-calibration REDESIGN** (multi-session, ACTIVE) — plan + findings:
  ...
  - [ ] **NEXT: native-checkerboard sub-pixel corner polish** — Phase-B showed ChESS-saddle corners ~0.6px vs opencv ~0.01px. Add a final refine (reuse `cv::SubPixelCornerRefiner`, or a saddle-specific parabolic/gradient polish on the ChESS response) → re-run Phase-B to confirm the gap closes.
