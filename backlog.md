...
- [ ] **`FixedMatrix<T,COLS,ROWS>` dim-order flip — STILL PENDING. DETAILED PLAN in `next.md` (execute in a fresh session).** MEASURED surface (much smaller than the raw ~581): **215 square** literal sites = NO change; **116 non-square** need a swap, of which **~94 are `1×N`/`N×1` vectors** (Phase A: convert to `FixedColVector`/`FixedRowVector` → order-agnostic, drop out of the flip) and **~17 genuine matrices** (`4×3`/`3×4`). Template params → no private-ctor forcing, BUT FixedMatrix ops are COMPILE-TIME dim-checked (most transposes fail to build) + a one-shot `static_assert(ROWS==COLS)` worklist + the 1010-suite are the net. NOTE: `FixedColVector`/`FixedRowVector` are SUBCLASSES not aliases (`FixedVector.h:12`) — their base-specs flip too. Execution: hybrid (delicate edits by hand, mechanical grind by a worktree subagent).
