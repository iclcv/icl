...
- [ ] **`FixedMatrix<T,COLS,ROWS>` dim-order flip — STILL PENDING (separate session).** Template param order still column-first (`FixedMatrix.h:48`); ripples through `FixedColVector`/`FixedRowVector` typedefs + `Vec`/`Mat` in `GeomDefs.h` → all of geom. ~581 instantiations, many NON-square (`1×3`, `3×4`, `4×3` camera/homography blocks, Jacobians). HIGHER RISK than DynMatrix: dims are TEMPLATE params so the private-ctor forcing trick does NOT apply (a `FixedMatrix<float,4,3>` still compiles transposed — silent). Needs grep + per-site number-swap + full-suite as the only net. Square (3×3, 4×4) are dimension-invariant. Scripts from the DynMatrix pass are in the scratchpad as a starting point.
