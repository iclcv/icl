...
namespace icl::physics2 {
  ...
  void FoldMap::clear() { m.fill(initialValue); }
  ...
  float FoldMap::getFoldValue(const utils::Point32f &a, const utils::Point32f &b) {
    ...
    for (int i = 1; i < r.n - 2; ++i) {
      const Point &p = r[i];
      if (static_cast<unsigned>(p.x) >= w || static_cast<unsigned>(p.y) >= h) continue;
      const float cc = c(p.x, p.y);
      if (cc >= 0) {
        if (cc < min) min = cc;
      } else {
        if (cc > maxNeg) { anyNeg = true; maxNeg = cc; }
      }
    }
    if (min >= 1) return anyNeg ? maxNeg : 1;
    return min;
  }
