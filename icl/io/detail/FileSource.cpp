...
namespace icl::io {
  ...
  FileSource::FileSource(const std::string &pattern,
                                   bool buffer,
                                   bool ignoreDesired)
    : m_data(new Data)
  {
    ...
    void FileSource::bufferImages(bool omitExceptions){
    ...
    unsigned int FileSource::getFileCount() const{

      return m_data->oFileList.size();
    }


    const std::string &FileSource::getNextFileName() const{
    ...
    core::Image FileSource::acquireImage(){
      try{
        ...
        // ImgBase) are mutated by FileSource on subsequent calls — hand
    ...
    const core::ImgBase *FileSource::grabDisplay(){
    ...
    void FileSource::processPropertyChange(const utils::Configurable::Property &prop){
      if(prop.name == "next") {
        next();
    ...
    void FileSource::updateProperties(const ImgBase* img){
