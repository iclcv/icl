...
namespace icl::io {
  ...
  struct Kinect2Source::Impl{
    LibFreenect2Context::Device *dev;
    Kinect2Source::Mode mode;
    ...
    std::pair<Time,Time> getTimesToCompare() {
      Time a, b;
      switch(mode){
        case Kinect2Source::GRAB_RGB_IMAGE:
          a = rgbImage.getTime();
          b = dev->rgbImage.getTime();
          break;
        case Kinect2Source::GRAB_DEPTH_IMAGE:
          a = depthImage.getTime();
          b = dev->depthImage.getTime();
          break;
        case Kinect2Source::GRAB_IR_IMAGE:
          a = irImage.getTime();
  ...
  Kinect2Source::Kinect2Source(Kinect2Source::Mode mode, int deviceID) {
    if(mode != DUMMY_MODE){
  ...
  const ImgBase* Kinect2Source::acquireImage(){
    ...
    case GRAB_DEPTH_IMAGE:{
      ...
      case GRAB_IR_IMAGE:
        ...
        break;
      default:
        throw ICLException("Kinect2Source::acquireImage() invalid image mode!");
        break;
    }
    return 0;
  }
  ...
  void Kinect2Source::processPropertyChange(const utils::Configurable::Property &prop){
  ...
  REGISTER_CONFIGURABLE(Kinect2Source, return new Kinect2Source(Kinect2Source::DUMMY_MODE));

  /// returns a list of attached kinect devices
  const std::vector<DeviceDescription> &Kinect2Source::getDeviceList(bool rescan){
