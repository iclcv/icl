...
namespace icl::io {
  static std::ostream &operator<<(std::ostream &s, const KinectSource::Mode &m){
  ...
  struct FreenectDevice{
    struct Used{
      ...
      KinectSource::Mode currentColorMode; // this must not be reset as long as the device is used somewhere else
      ...
      const ImgBase &getLastColorImage(bool avoidDoubleFrames){
        std::scoped_lock lock(colorMutex);
        ImgBase &src = ( currentColorMode == KinectSource::GRAB_RGB_IMAGE ? (ImgBase&)colorImage :
                         currentColorMode == KinectSource::GRAB_IR_IMAGE_8BIT ? (ImgBase&)irImage :
                         (ImgBase&)irImage16s);
        ImgBase &dst = ( currentColorMode == KinectSource::GRAB_RGB_IMAGE ? (ImgBase&)colorImageOut :
                         currentColorMode == KinectSource::GRAB_IR_IMAGE_8BIT ? (ImgBase&)irImageOut :
        ...
        if((currentColorMode != KinectSource::GRAB_RGB_IMAGE) && (irShift != Off)){
    ...
    Used *used;
    KinectSource::Mode mode;
    int index;

    void setMode(KinectSource::Mode mode, Used *used, const Size &size){
      if(mode != KinectSource::GRAB_DEPTH_IMAGE){
        if(used->currentColorMode != KinectSource::GRAB_DEPTH_IMAGE
          ...
          used->currentColorMode = mode;
        }else if(used->currentColorMode == KinectSource::GRAB_DEPTH_IMAGE){
          used->currentColorMode = mode;
        }
      }
      ...
      const bool isVideo = (mode != KinectSource::GRAB_DEPTH_IMAGE);
    ...
    FreenectDevice(int index, KinectSource::Mode mode, Size size):mode(mode),index(index){
      ...
      if(it == devices.end()){
        ...
        if(mode != KinectSource::GRAB_DEPTH_IMAGE){
          ...
          if(mode == KinectSource::GRAB_RGB_IMAGE){
            used->colorImage = Img8u(size,formatRGB);
          }else if(mode == KinectSource::GRAB_IR_IMAGE_8BIT){
            used->irImage = Img8u(size,1);
          }else if(mode == KinectSource::GRAB_IR_IMAGE_10BIT){
    ...
    ~FreenectDevice() noexcept (false) {
      if(mode != KinectSource::GRAB_DEPTH_IMAGE){
        used->numColorUsers--;
        if(!used->numColorUsers){
          used->currentColorMode = KinectSource::GRAB_DEPTH_IMAGE;
  ...
  public:
    ...
    Impl(KinectSource::Mode mode, std::string idOrSerial, const Size &size)
    ...
    void switchMode(KinectSource::Mode mode, const Size &size){
  ...
  KinectSource::KinectSource(KinectSource::Mode mode, int deviceID, const Size &size) {
    init(mode, str(deviceID),size);
  }
  KinectSource::KinectSource(KinectSource::Mode mode, const std::string &idOrSerial, const Size &size) {
    init(mode, idOrSerial,size);
  }

  void KinectSource::init(KinectSource::Mode mode, const std::string &idOrSerial, const Size &size){
  ...
  const ImgBase* KinectSource::acquireImage(){
  ...
  void KinectSource::processPropertyChange(const utils::Configurable::Property &prop){
    std::scoped_lock lock(m_impl->mutex);
  ...
  REGISTER_CONFIGURABLE(KinectSource, return new KinectSource(KinectSource::GRAB_DEPTH_IMAGE, 0, utils::Size::VGA));

  /// returns a list of attached kinect devices
  const std::vector<DeviceDescription> &KinectSource::getDeviceList(bool rescan){
    ...
    if(rescan){
      ...
      for(size_t i=0;i<ctx.deviceAssociation.size();++i){
        try{
          KinectSource g(GRAB_RGB_IMAGE,i);
