...
#define XI_CALL(X) DEBUG_LOG("calling xiFunction " << #X); X
    ...
    struct XiSource::Data{
      ...
      void addRangeProperty(const std::string &type, XiSource *g, const std::string &what,
      ...
      static void handle_result(XI_RETURN s, const std::string &where,
        ...
        if(s != XI_OK){
          ...
          std::string errorText = "XiSource: error in XiApi in " + where + " (error code was: "
            + str(s)  + ":" + errors[s] + ")";
          if(throwException){
    ...
    void XiSource::init(int deviceID){
    ...
    XiSource::XiSource(int deviceID) : m_data(0){
      init(deviceID);
    ...
    const std::vector<DeviceDescription> &XiSource::getDeviceList(std::string, bool rescan){
    ...
    core::Image XiSource::acquireImage(){
    ...
    void XiSource::processPropertyChange(const utils::Configurable::Property &prop){
      XI_RETURN s;
