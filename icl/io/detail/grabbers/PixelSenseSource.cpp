...
#define TIMEOUT 1000
  ...
  std::ostream &operator<<(std::ostream &str, const PixelSenseSource::Blob &b){
    return str << ((const ps_blob&)b);
  }

  std::istream &operator>>(std::istream &str, PixelSenseSource::Blob &b){
  ...
  PixelSenseSource::PixelSenseSource(float maxFPS):m_data(new Data){
  ...
  PixelSenseSource::~PixelSenseSource(){
    delete m_data;
  }
  ...
  core::Image PixelSenseSource::acquireImage(){
  ...
  std::vector<PixelSenseSource::Blob>
  PixelSenseSource::extractBlobMetaData(const ImgBase *image){
    ICLASSERT_THROW(image, ICLException("PixelSenseSource::extractBlobMetaData: image is null"));
