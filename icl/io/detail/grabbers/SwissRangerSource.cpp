...
#undef ICL_INSTANTIATE_DEPTH
  ...
  class SwissRangerSource::SwissRanger{
    public:
      SRCAM cam;
      Size size;
      Img32f buf;
  ...
  SwissRangerSource::SwissRangerSource(int serialNumber, depth bufferDepth, int pickChannel):SourceBackend(){
  ...
  SwissRangerSource::~SwissRangerSource(){
  ...
  void SwissRangerSource::addProperties(){
    ...
    for (int i = 0; i < 8; ++i){
      ...
      addProperty(props[i], prop::Flag{}, (id & curMode) ? true : false, "");
    }
    addProperty("intensity-image-mode", prop::Menu{"zero", "minus one", "unchanged"}, imgmode, "");
    ...
    addProperty("modulation-frequency",
                prop::Menu{"40Mhz", "30MHz", "21MHz", "20MHz", "19MHz",
                           ...
                           "14_5MHz", "15_5MHz"},
                modfreq, "");
    addProperty("depth-map-unit", prop::Menu{"16Bit", "mm", "cm", "m"}, m_sr->depthMapUnit, "");
    addProperty("current-range", prop::Info{}, currentrange, "");
    addProperty("create-xyz-channels", prop::Flag{}, m_sr->createXYZ, "");
    addProperty("format", prop::Info{}, "not used", "");
    addProperty("size", prop::Info{}, "not used", "");

    registerCallback([this](const utils::Configurable::Property &p){ processPropertyChange(p); });
  }

  // callback for changed configurable properties
  void SwissRangerSource::processPropertyChange(const utils::Configurable::Property &prop){
    std::scoped_lock l(m_mutex);
  ...
  float SwissRangerSource::getMaxRangeMM(const std::string &modulationFreq){
    return get_max_range_mm(translate_modulation_freq(modulationFreq));
  }

  float SwissRangerSource::getMaxRangeVal() const{
  ...
  core::Image SwissRangerSource::acquireImage(){
    std::scoped_lock l(m_mutex);
    SR_Acquire(m_sr->cam);
...
m_sr->iim == iimUnknownPixelsZero ? 0 : -1); break;
  ...
  const std::vector<DeviceDescription> &SwissRangerSource::getDeviceList(std::string hint, bool rescan){
