...
namespace icl::io {
  ...
  class V4L2Source::Impl : public Thread {
    public:
      struct V4L2Buffer{
          void *data;
          size_t length;
      ...
      Impl(const std::string &deviceName, const std::string &initialFormat="", bool startGrabbing=true):
        ...
        if(deviceName.length() == 1 && match(deviceName,"^[0-9]\\b")){
          if(File("/dev/video/"+deviceName).exists()){
            ...
            std::cout << "V4L2Source added device prefix '/dev/video/' automatically" << std::endl;
          }else if(File("/dev/video"+deviceName).exists()){
            this->deviceName = "/dev/video"+deviceName;
            std::cout << "V4L2Source added device prefix '/dev/video' automatically" << std::endl;
          }else{
            throw ICLException("V4L2Source initialization unable to find video device (given device ID: "
      ...
      */
      ...
      SupportedPropertyPtr findProperty(const std::string &name){
        ...
        //if(it == supportedProperties.end()) throw ICLException("V4L2Source: unknown property '" + name + "'");
  ...
  V4L2Source::V4L2Source(const std::string &device)
    : implMutex()
  {
  ...
  const ImgBase *V4L2Source::acquireImage(){
    std::scoped_lock lock(implMutex);
  ...
  void V4L2Source::processPropertyChange(const utils::Configurable::Property &prop){
    std::scoped_lock lock(implMutex);
    if(prop.name == "format"){
  ...
  const std::vector<DeviceDescription> &V4L2Source::getDeviceList(std::string hint, bool rescan){
