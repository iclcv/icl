...
#include <icl/io/detail/opencv/OpenCVVideoSource.h>
...
namespace icl::io {
  ...
  Image OpenCVVideoSource::acquireImage(){
  ...
  OpenCVVideoSource::OpenCVVideoSource(const std::string &fileName) : data(new Data), mutex(), updating(false){
  ...
  void OpenCVVideoSource::processPropertyChange(const utils::Configurable::Property &prop){
    std::scoped_lock l(mutex);
    if(updating) return;
  ...
  REGISTER_CONFIGURABLE(OpenCVVideoSource, return new OpenCVVideoSource(""));

  SourceBackend* createCVVSource(const std::string &param){
    return new OpenCVVideoSource(param);
  }
  ...
  const std::vector<DeviceDescription>& getOCVVDeviceList(std::string hint, bool rescan){
    ...
    deviceList.clear();
    // if filter exists, add source with filter
    if(hint.size()) deviceList.push_back(
      DeviceDescription("cvvideo", hint, "A source for opencv-videos.")
      );
    return deviceList;
  }

  REGISTER_SOURCE_BACKEND(cvvideo,createCVVSource, getOCVVDeviceList, "video filename~OpenCV based video file source");
