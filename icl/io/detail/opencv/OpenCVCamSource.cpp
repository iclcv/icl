// SPDX-License-Identifier: LGPL-3.0-or-later
// ICL - Image Component Library (https://github.com/iclcv/icl)
// Copyright (C) 2006-2026 Christian Groszewski, Christof Elbrechter, V. Richter

#include <icl/io/detail/opencv/OpenCVCamSource.h>
#include <icl/utils/prop/Constraints.h>
#include <mutex>

using namespace icl::utils;
using namespace icl::core;

namespace icl::io {
  OpenCVCamSource::OpenCVCamSource(int dev) :device(dev),m_buffer(0){
    cvc.reset(new cv::VideoCapture());
    cvc->open(dev);
    if(!cvc->isOpened()){
      throw ICLException("unable to create OpenCVCamSourceImpl with device index "
                         + str(dev) + ": invalid device ID");
    }
    addProperty("size",
                prop::Menu{"160x120","320x200","320x240","480x320","640x350",
                           "640x480","800x480","800x600","960x540","960x640",
                           "1024x768","1152x864","1200x800","1280x720","1280x800",
                           "1440x900","1280x960","1280x1024","1600x900","1400x1050",
                           "1600x1050","1600x1200"},
                str(cvc->get(cv::CAP_PROP_FRAME_WIDTH))+"x"
                +str(cvc->get(cv::CAP_PROP_FRAME_HEIGHT)), "");
    addProperty("brightness", prop::Range{.min=0, .max=100, .step=1}, 
                cvc->get(cv::CAP_PROP_BRIGHTNESS), "");
    addProperty("contrast", prop::Range{.min=0, .max=100, .step=1}, 
                cvc->get(cv::CAP_PROP_CONTRAST), "");
    addProperty("saturation", prop::Range{.min=0, .max=100, .step=1}, 
                cvc->get(cv::CAP_PROP_SATURATION), "");
    addProperty("hue", prop::Range{.min=0, .max=100, .step=1}, 
                cvc->get(cv::CAP_PROP_HUE), "");
    addProperty("format", prop::Menu{"RGB"}, "RGB", "");
    registerCallback(
          [this](const utils::Configurable::Property &p){ processPropertyChange(p); });
  }

  OpenCVCamSource::~OpenCVCamSource() = default;

  Image OpenCVCamSource::acquireImage(){
    ICLASSERT_RETURN_VAL( !(cvc==0), Image());
    cv::Mat frame;
    cvc->read(frame);
    // OpenCV captures in BGR; convert to RGB for ICL
    cv::cvtColor(frame, frame, cv::COLOR_BGR2RGB);
    std::scoped_lock lock(m_mutex);
    ImgBase *raw = m_buffer.ptr();
    core::mat_to_img(&frame, &raw);
    if(raw != m_buffer.ptr()) m_buffer = Image(raw);
    return m_buffer;
  }

  // callback for changed configurable properties
  void OpenCVCamSource::processPropertyChange(const utils::Configurable::Property &prop){
    std::scoped_lock lock(m_mutex);
    if(prop.name == "size"){
      cvc.reset(new cv::VideoCapture());
      cvc->open(device);
      Size s(prop.as<std::string>());
      cvc->set(cv::CAP_PROP_FRAME_WIDTH,double(s.width));
      cvc->set(cv::CAP_PROP_FRAME_HEIGHT,double(s.height));

      /* TODO: acatually, the resulting image sizes are not used ??
          m_bIgnoreDesiredParams = false;
          if(i==0 || j==0)
          setDesiredSize(s);
      */
    }else if(prop.name == "brightness"){
      cvc->set(cv::CAP_PROP_BRIGHTNESS,prop.as<double>()*0.01);
    }else if(prop.name == "contrast"){
      cvc->set(cv::CAP_PROP_CONTRAST,prop.as<double>()*0.01);
    }else if(prop.name == "saturation"){
      cvc->set(cv::CAP_PROP_SATURATION,prop.as<double>()*0.01);
    }else if(prop.name == "hue"){
      cvc->set(cv::CAP_PROP_HUE,prop.as<double>()*0.01);
    }
  }

  const std::vector<DeviceDescription> &OpenCVCamSource::getDeviceList(std::string hint, bool rescan){
    static std::vector<DeviceDescription> deviceList;
    if(rescan){
      deviceList.clear();
      for(int i=0;i<100;++i){
        try{
          OpenCVCamSource g(i);
          deviceList.push_back(DeviceDescription("cvcam",str(i),"OpenCV SourceBackend Device "+str(i)));
        }catch(ICLException &e){
          break;
        }
      }
    }
    return deviceList;
  }

  REGISTER_CONFIGURABLE(OpenCVCamSource, return new OpenCVCamSource(0));

  SourceBackend* createCVCSource(const std::string &param){
    return new OpenCVCamSource(to32s(param));
  }

  REGISTER_SOURCE_BACKEND(cvcam,createCVCSource, OpenCVCamSource::getDeviceList, "camera ID~OpenCV based camera source");

  } // namespace icl::io