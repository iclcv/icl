...
static LLM llm(1,1);
static constexpr float MINX = 0, MAXX = 100;
static constexpr float MINY = -10, MAXY = 10;

HBox gui;

float tx(float x){ return (x-MINX)/(MAXX-MINX); }
float ty(float y){ return (y-MINY)/(MAXY-MINY); }
...
void run(){
  ...
  if(reset.wasTriggered()){
    llm = LLM(1,1);
    llm.init(gui["kernel-count"], {Range<icl32f>(MINX,MAXX)}, {5});
  }

  draw->rel();

  // draw the target function (red) and net output (green)
  constexpr int STEPS = 100;
  constexpr float DX = (MAXX-MINX)/STEPS;
  float xLast = 0, yLast = 0, ynetLast = 0;
  ...
  // draw kernel positions (yellow) and variance (magenta)
