...
void init(){
  ...
  gui << Display({.handle="orig-image", .label="Original", .minSize={20, 15}})
      << Display({.handle="net-image", .label="Net Output", .minSize={20, 15}})
      << ( VBox({.minSize={15, 0}})
           << Button("Train Step", {.handle="train"})
           << ToggleButton("Train Off", "Train On", false, {.handle="train-loop"})
           << ToggleButton("NO Soft-Max", "Soft-Max", false, {.handle="use-soft-max"})
