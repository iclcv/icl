...
#include <icl/markers/MarkerGridBasedUndistortionOptimizer.h>
...
void init(){
  ...
  gui << ( Tab("distorted input,undistorted image,"
      ...
      << ( VBox({.minSize={14, 1}, .maxSize={14, 99}})
           << Fps(10, {.handle="fps", .minSize={1, 2}, .maxSize={99, 2}})
           << ( HBox()
                << Label("--", {.handle="error", .label="src error", .minSize={1, 2}, .maxSize={99, 2}})
                << Label("--", {.handle="uerror", .label="fixed error", .minSize={1, 2}, .maxSize={99, 2}})
                )
           << ( HBox()
                << CamCfg({.minSize={1, 2}, .maxSize={99, 2}})
                << Combo(fd->getIntermediateImageNames(), {.handle="vis", .maxSize={99, 2}})
              )
           << CheckBox("show detection overlay", {.checked=true, .handle="overlay"})
           << ( Tab("udist,markers,optimize")
                ...
                << (VBox()
                    ...
                    << CheckBox("Use OpenCL", {.checked=true, .handle="use opencl"})
