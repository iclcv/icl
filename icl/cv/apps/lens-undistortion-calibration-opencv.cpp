...
void init(){
  ...
  gui << ( VSplit().label("image")
      ...
      << ( VBox().label("controls").minSize(15,1)
           ...
           << (HBox()
               << Slider(5, maxMarkers, 5).hideIf(!pa("-g")).handle("minMarkers")
