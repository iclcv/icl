...
void init(){
  ...
  gui << ui::Canvas({.handle="image", .label="image", .minSize={32, 24}})
      << ( ui::VBox({.label="Region information"})
           << ui::Label({.handle="total", .label="Total Region Count", .minSize={6, 2}})
           << ( ui::HBox()
                << ui::Label({.handle="size-handle", .label="Region Size", .minSize={6, 2}})
                << ui::Label({.handle="cog-handle", .label="Region COG", .minSize={6, 2}})
                )
           << ( ui::HBox()
                << ui::Label({.handle="val-handle", .label="Region Value", .minSize={6, 2}})
                << ui::Label({.handle="ff-handle", .label="Region Form Factor", .minSize={6, 2}})
                )
           << ( ui::HBox()
                << ui::Label({.handle="evratio-handle", .label="Region EV-Ratio", .minSize={6, 2}})
                << ui::Label({.handle="bl-handle", .label="Region Boundary Length", .minSize={6, 2}})
                )
           << ( ui::HBox()
                << ui::Label({.handle="nSub", .label="sub regions", .minSize={6, 2}})
                << ui::Label({.handle="nAllSub", .label="all sub regions", .minSize={6, 2}})
                )
           << ( ui::HBox()
                << ui::CheckBox("show boundary", {.checked=true, .handle="showBoundary"})
                << ui::ToggleButton("normal", "thinned", true, {.handle="showThinnedBoundary"})
                )
           << ( ui::HBox()
                << ui::CheckBox("show sub regions", {.checked=true, .handle="showSubRegions"})
                << ui::ToggleButton("direct", "all", false, {.handle="showAllSubRegions"})
                )
           << ( ui::HBox()
                << ui::CheckBox("show sur. regions", {.checked=true, .handle="showSurRegions"})
                << ui::ToggleButton("direct", "all", false, {.handle="showAllSurRegions"})
                )
           << ( ui::HBox()
                << ui::CheckBox("show neighbours", {.handle="showNeighbours"})
                << ui::CheckBox("show bounding rect", {.handle="showBB"})
                )
           << ( ui::HBox()
                << ui::ToggleButton("stopped", "grabbing", true, {.handle="grabbing", .minSize={3, 2}})
                << ui::Button("grab next", {.handle="grab-next-handle", .minSize={3, 2}})
                )
           << ui::Slider(2, 256, 10, {.handle="levels", .label="reduce levels"})
           << ui::Slider(1, 10, 0, {.handle="medianSize", .label="preprocessing median mask size"})
           << ui::Label("---", {.handle="timeRD", .label="time for region detection"})
           << ui::Label("---", {.handle="timeNB", .label="time for neighbour detection"})
           << ui::Label("---", {.handle="timeSR", .label="time for sub region detection"})
           << ui::Label("---", {.handle="timeSU", .label="time for sur. region detection"})
           )
      << ui::Show();
