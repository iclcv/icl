...
namespace icl::cv {
  ...
  public:
    struct Params {
      ...
      /// Parallelize the dense response + gray conversion with OpenMP (no effect
      /// if ICL was built without OpenMP). Results are identical either way.
      /// Disable when running inside an already-parallel context (e.g. many
      /// detectors at once) to avoid thread oversubscription.
