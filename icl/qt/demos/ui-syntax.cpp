...
// (`icl::qt::ui::`).  After Phase 4 the whole layout — containers AND
// leaves — is expressed in `ui::` form.  Containers inherit from their
...
void init(){
  gui << ( ui::VBox({.spacing=2})
           << ( ui::HBox({.label="numeric inputs"})
                << ui::Slider(0, 255, 42,
