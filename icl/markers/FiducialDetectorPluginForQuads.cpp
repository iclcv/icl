...
namespace icl::markers {
  ...
  FiducialDetectorPluginForQuads::FiducialDetectorPluginForQuads():
    ...
    addProperty("max tilt",prop::Range{.min=1, .max=200}, 150, "Maximum tilt for quad markers (1: only perfect squares are\n"
                "detected, 5-10 most tilted quads are detected, >10\n"
                "also very tilted rectangular quads are detected (not linear).");
    addProperty("return rejected quads",prop::Flag{}, false, "If set to true, also quads that were not identified as valid markers\n"
