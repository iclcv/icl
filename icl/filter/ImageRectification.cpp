...
namespace icl::filter {
    ...
    static const Homography2D create_and_check_homography(bool validateAndSortPoints,const Point32f psin[4], const Img<T> &src,
      ...
      if(Q || R || (maxTilt > 0)){
        ...
        FixedMatrix<float,2,2>(HOM(0, 0),HOM(0, 1),
                               HOM(1, 0),HOM(1, 1)).decompose_QR(q,r);
        if(Q) *Q = q;
        if(R) *R = r;
        if(maxTilt>0){
    ...
    const Img<T> &apply_nearest_neighbor(const Img<T> &src,
      ...
      const __m128 hom01 = _mm_set1_ps(hom(1, 0));
      const __m128 hom02 = _mm_set1_ps(hom(2, 0));
      const __m128 hom10 = _mm_set1_ps(hom(0, 1));
      const __m128 hom11 = _mm_set1_ps(hom(1, 1));
      const __m128 hom12 = _mm_set1_ps(hom(2, 1));
      const __m128 hom20 = _mm_set1_ps(hom(0, 2));
      const __m128 hom21 = _mm_set1_ps(hom(1, 2));
