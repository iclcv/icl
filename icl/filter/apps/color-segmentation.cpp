...
void init(){
  ...
  gui << ( HSplit()
           << Canvas({.handle="image", .label="camera image", .minSize={16, 12}})
           << Canvas({.handle="seg", .label="segmentation result", .minSize={16, 12}})
           )
      << ( HSplit()
           << (Tab("2D,3D", {.handle="tab"})
               << ( HBox()
                    << Canvas({.handle="lut", .label="lut", .minSize={16, 12}})
                    << ( VBox({.minSize={4, 1}, .maxSize={3, 100}})
                         << Combo("x,y,z", {.handle="zAxis"})
                         << Slider(0, 255, 0, {.vertical=true, .handle="z", .label="vis. plane"})
                         )
                    )
               << ( HBox()
                    << Canvas3D(Size::VGA, {.handle="lut3D"})
                    << Slider(0, 255, 200, {.vertical=true, .handle="alpha", .label="alpha", .maxSize={2, 100}})
                  )
               )
           << ( VBox()
                << CheckBox("Pause Image Acquisition", {.checked=false, .handle="paused"})
                << ( HBox()
                     << Combo(classes.str(), {.handle="currClass", .label="current class"})
                     << ToggleButton("current class", "background", false, {.handle="lb", .label="left button"})
                   )
                << Slider(0, 255, 4, {.handle="radius", .label="color radius"})

                << (HBox({.label="smooth LUT"})
                    << Slider(0, 27, 10, {.handle="smoothThresh", .label="threshold"})
                ...
                << ( HBox()
                     << CheckBox("pre median", {.handle="preMedian"})
                     << CheckBox("post median", {.handle="postMedian"})
                   )
                << ( HBox()
                     << CheckBox("post dilation", {.handle="postDilatation"})
                     << CheckBox("post erosion", {.handle="postErosion"})
                   )
                << ( HBox()
