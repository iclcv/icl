...
void init(){
  ...
  gui << ( ui::HSplit()
           << ui::Canvas({.handle="image", .label="camera image", .minSize={16, 12}})
           << ui::Canvas({.handle="seg", .label="segmentation result", .minSize={16, 12}})
           )
      << ( ui::HSplit()
           << (ui::Tab("2D,3D", {.handle="tab"})
               << ( ui::HBox()
                    << ui::Canvas({.handle="lut", .label="lut", .minSize={16, 12}})
                    << ( ui::VBox({.minSize={4, 1}, .maxSize={3, 100}})
                         << ui::Combo("x,y,z", {.handle="zAxis"})
                         << ui::Slider(0, 255, 0, {.vertical=true, .handle="z", .label="vis. plane"})
                         )
                    )
               << ( ui::HBox()
                    << ui::Canvas3D(Size::VGA, {.handle="lut3D"})
                    << ui::Slider(0, 255, 200, {.vertical=true, .handle="alpha", .label="alpha", .maxSize={2, 100}})
                  )
               )
           << ( ui::VBox()
                << ui::CheckBox("Pause Image Acquisition", {.checked=false, .handle="paused"})
                << ( ui::HBox()
                     << ui::Combo(classes.str(), {.handle="currClass", .label="current class"})
                     << ui::ToggleButton("current class", "background", false, {.handle="lb", .label="left button"})
                   )
                << ui::Slider(0, 255, 4, {.handle="radius", .label="color radius"})

                << (ui::HBox({.label="smooth LUT"})
                    << ui::Slider(0, 27, 10, {.handle="smoothThresh", .label="threshold"})
                    << ui::Button("do it", {.handle="smooth"})
                    )
                << ( ui::HBox()
                ...
                << ( ui::HBox()
                     << ui::CheckBox("pre median", {.handle="preMedian"})
                     << ui::CheckBox("post median", {.handle="postMedian"})
                   )
                << ( ui::HBox()
                     << ui::CheckBox("post dilation", {.handle="postDilatation"})
                     << ui::CheckBox("post erosion", {.handle="postErosion"})
                   )
                << ( ui::HBox()
                     << ui::Label("?", {.handle="time", .label="time for segm."})
                     << ui::Fps(10, {.handle="fps", .label="system fps"})
                   )
                << ( ui::HBox()
                     << ui::CamCfg()
