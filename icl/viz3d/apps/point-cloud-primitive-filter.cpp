...
// point-cloud-primitive-filter (viz3d): keep/remove the points of a depth/RGBD
...
// viz3d Primitive3D→node converter); the filtering uses the reusable viz3d
...
// RSB is being retired, and viz3d already carries the box/sphere filters. (Per
...
void run() {
  ...
  // show the primitive via the viz3d converter (rebuild each frame — cheap)
