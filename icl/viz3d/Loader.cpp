...
namespace icl::viz3d {
  ...
  static std::vector<std::shared_ptr<MeshNode>> loadGltf(const std::string &filename) {
    ...
    for (cgltf_size ni = 0; ni < data->nodes_count; ni++) {
      ...
      for (cgltf_size pi = 0; pi < node->mesh->primitives_count; pi++) {
        ...
        Mat T;
        T(0, 0)=m[0]; T(0, 1)=m[1]; T(0, 2)=m[2];  T(0, 3)=m[12];
        T(1, 0)=m[4]; T(1, 1)=m[5]; T(1, 2)=m[6];  T(1, 3)=m[13];
        T(2, 0)=m[8]; T(2, 1)=m[9]; T(2, 2)=m[10]; T(2, 3)=m[14];
  ...
  static std::vector<std::shared_ptr<MeshNode>> loadObj(const std::string &filename) {
    ...
    if (!file.is_open()) {
      fprintf(stderr, "[viz3d::loadFile] cannot open: %s\n", filename.c_str());
      return {};
    }
  ...
  std::vector<std::shared_ptr<MeshNode>> loadFile(const std::string &filename) {
    ...
    if (ext == ".glb" || ext == ".gltf") {
      ...
      fprintf(stderr, "[viz3d::loadFile] unsupported format: %s\n", ext.c_str());
      return {};
    }
  }
