...
#include <icl/viz3d/render/Raytracer.h>
#include <memory>
#include <string>
...
namespace icl::viz3d {
  ...
  ///   cyc.setSceneScale(1.0f);     // viz3d is in mm — see "Scene scale" below
  ///   cyc.setSamples(128);
  ///   cyc.renderBlocking(0);       // runs all samples, then returns
  ///   io::save(cyc.getImage(), "out.png");
  /// \endcode
  /// See demo `viz3d-headless-cycles-capture` for the full minimal program.
  ///
  ...
  ///   • Scene scale: viz3d units are MILLIMETRES, but Cycles' default scale is
  ...
  public:
    ...
    /// World units per viz3d unit. viz3d is in MILLIMETRES; Cycles' default is
