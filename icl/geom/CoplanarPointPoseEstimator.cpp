...
namespace icl::geom {
...
#endif
    addProperty("RANSAC.enable", prop::Flag{}, data->ransacSpec.useRANSAC, "Enable RANSAC sampling. This is only needed when more than 5 points are "
                "used and when these points are prone to outliers");
    addProperty("RANSAC.num points for model",prop::Range{.min=4, .max=100, .step=1}, 
                data->ransacSpec.numPointsForModel, "Number of points used for generating models during RANSAC sampling");
    addProperty("RANSAC.number of cycles",prop::Range{.min=10, .max=10000, .step=1}, 
                data->ransacSpec.numRandomCycles, "Number of RANSAC cycles performed");
    addProperty("RANSAC.max projection distance",prop::Range{.min=0, .max=300}, 
                data->ransacSpec.maxPointProjectionDistance, "Maximum projection error for points to be classified as inlier");
  ...
  static Mat optimize_error(const Mat &P, const Mat &T_initial, const Point32f *M, const Point32f *I, int n,
    ...
    FixedMatrix<icl32f, 3, 3> getRzyx(icl32f a, icl32f b, icl32f c) {
      ...
      R(1, 0) = cosB * sinC;
      R(2, 0) = -sinB;
      R(0, 1) = sinAcosC * sinB - cosAsinC;
      R(1, 1) = sinAsinC * sinB + cosAcosC;
      R(2, 1) = sinA * cosB;
      R(0, 2) = cosAcosC * sinB + sinAsinC;
      R(1, 2) = cosAsinC * sinB - sinAcosC;
      R(2, 2) = cosA * cosB;
    ...
    void RzyxToAngles(FixedMatrix<icl32f, 3, 3> &R, FixedMatrix<icl32f, 1, 3> &res0, FixedMatrix<icl32f, 1, 3> &res1) {
      if (R(2, 0) > -0.999999 && R(2, 0) < 0.999999) {
        ...
        double cosB0 = cos(res0(1, 0));
        double cosB1 = cos(res1(1, 0));

        res0(0, 0) = atan2(R(2, 1) / cosB0, R(2, 2) / cosB0);
        res1(0, 0) = atan2(R(2, 1) / cosB1, R(2, 2) / cosB1);

        res0(2, 0) = atan2(R(1, 0) / cosB0, R(0, 0) / cosB0);
        res1(2, 0) = atan2(R(1, 0) / cosB1, R(0, 0) / cosB1);

      } else {
        res0(0, 0) = res1(0, 0) = 0.0;
        if (R(2, 0) < 0.0) {
          res0(1, 0) = res1(1, 0) = M_PI / 2.0;
          res0(2, 0) = res1(2, 0) = atan2(R(0, 1), R(0, 2));
        } else {
          res0(1, 0) = res1(1, 0) = M_PI / -2.0;
          res0(2, 0) = res1(2, 0) = atan2(-R(0, 1), -R(0, 2));
        }
    ...
    void calcRt(const int n, std::vector<FixedMatrix<icl32f, 1, 3> > &v, FixedMatrix<icl32f, 3, 3> &Rt) {
      ...
      for (int i = 0; i < n; ++i) {
        double len = 1 / sqrt(pow(v[i](0, 0), 2) + pow(v[i](1, 0), 2) + 1);
        x += v[i](0, 0) * len;
        y += v[i](1, 0) * len;
        z += len;
      }
  ...
  void CoplanarPointPoseEstimator::robustPoseCorrection(int n, const Point32f *modelPoints,
    ...
    Rdz = getRzyx(0.0f, 0.0f, atan2(R_(2, 1), R_(2, 0)));
    R2 = R_ * Rdz;
    ...
    if (fabs(angles0[0]) <= M_PI / 2) {
      Ry = getRzyx(0.0f, angles0(1, 0), 0.0f);
      Rz = getRzyx(0.0f, 0.0f, angles0(2, 0));
    } else {
      Ry = getRzyx(0.0f, angles1(1, 0), 0.0f);
      Rz = getRzyx(0.0f, 0.0f, angles1(2, 0));
    }
  ...
  // tested the homography error which is always almost 0
    ...
    data->T(3, 0) = data->T(3, 1) = data->T(3, 2) = 0;
    data->T(3, 3) = 1;
