...
// Copyright (C) 2006-2026 Christof Elbrechter, Patrick Nobou, Andre Ueckermann
...
namespace icl::geom {
  ...
  inline Point map_rgbd(const Mat &M, const ViewRayDir &v){
    const float phInv = 1.0/ ( M(3, 0) * v[0] + M(3, 1) * v[1] + M(3, 2) * v[2] + M(3, 3) );
    const int px = phInv * ( M(0, 0) * v[0] + M(0, 1) * v[1] + M(0, 2) * v[2] + M(0, 3) );
    const int py = phInv * ( M(1, 0) * v[0] + M(1, 1) * v[1] + M(1, 2) * v[2] + M(1, 3) );
    return Point(px,py);
  }
