...
ICLPWCGrabber::ICLPWCGrabber(int iWidth,
                             int iHeight,
                             icldepth eDepth,
                             iclformat eFormat,
                             float fFps,
...
void ICLPWCGrabber::grab(ICLBase *poOutput){
  ...
  unsigned char *pucPwcData = usbvflg_buf[iDevice] + usbvflg_vmbuf[iDevice].offsets[use_frame];
  
 
  if(poOutput->getFormat() == formatRGB && poOutput->getDepth() == depth8u){
 
    unsigned char *pU = pucPwcData+iWidth*iHeight;
    unsigned char *pV = pucPwcData+(int)(1.25*iWidth*iHeight);
    int iW2 = iWidth/2;
    ...
    unsigned char *pUDst = pucFlippedYUVData+iWidth*iHeight;
    unsigned char *pVDst = pucFlippedYUVData+(int)(1.25*iWidth*iHeight);
 
    // ?? mirror just in the rgb-8 case ?
    MIRROR_HORZ(pucPwcData,iWidth*sizeof(uchar),pucFlippedYUVData,oSize);
    MIRROR_HORZ(pU,iW2*sizeof(uchar),pUDst,oSize2);
    MIRROR_HORZ(pV,iW2*sizeof(uchar),pVDst,oSize2);
    
    convertYUV420ToRGB8(poOutput->asIcl8u(),pucFlippedYUVData,iWidth,iHeight);
 
  }else{
    convertYUV420ToRGB8(poRGB8Image->asIcl8u(),pucPwcData,iWidth,iHeight);
    iclcc(poOutput,poRGB8Image);
  }
