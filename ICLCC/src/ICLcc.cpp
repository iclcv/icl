...
// {{{ convert,set, and copy channels using IPP#define CONVERT_8U32F(S,SC,D,DC) ippiConvert_8u32f_C1R(S->asImg<iclbyte>()->getROIData(SC),S->getLineStep(),D->asImg<iclfloat>()->getROIData(DC),D->getLineStep(),D->getROISize())#define CONVERT_32F8U(S,SC,D,DC) ippiConvert_32f8u_C1R(S->asImg<iclfloat>()->getROIData(SC),S->getLineStep(),D->asImg<iclbyte>()->getROIData(DC),D->getLineStep(),D->getROISize(),ippRndZero)   #define SET_32F(V,D,DC) ippiSet_32f_C1R(V,D->asImg<iclfloat>()->getROIData(DC),D->getLineStep(),D->getROISize());#define SET_8U(V,D,DC) ippiSet_8u_C1R(V,D->asImg<iclbyte>()->getROIData(DC),D->getLineStep(),D->getROISize());#define COPY_8U(S,SC,D,DC) ippiCopy_8u_C1R(S->asImg<iclbyte>()->getROIData(SC),S->getLineStep(),D->asImg<iclbyte>()->getROIData(DC),D->getLineStep(),D->getROISize())  #define COPY_32F(S,SC,D,DC) ippiCopy_32f_C1R(S->asImg<iclfloat>()->getROIData(SC),S->getLineStep(),D->asImg<iclfloat>()->getROIData(DC),D->getLineStep(),D->getROISize())  // }}}
#else
// {{{ convert,set, and copy channels using C++#define CONVERT_8U32F(S,SC,D,DC)                                                        \    for(int x=0;x<S->getWidth();x++){                                                   \       for(int y=0;y<S->getHeight();y++){                                               \         (*(S->asImg<iclbyte>()))(x,y,SC)=static_cast<iclbyte>((*(D->asImg<iclfloat>()))(x,y,DC));      \       }                                                                                \    }#define CONVERT_32F8U(S,SC,D,DC)                                                        \    for(int x=0;x<S->getWidth();x++){                                                   \       for(int y=0;y<S->getHeight();y++){                                               \         (*(S->asImg<iclfloat>()))(x,y,SC)=static_cast<iclfloat>((*(D->asImg<iclbyte>()))(x,y,DC));     \       }                                                                                \    }#define SET_32F(V,D,DC) D->asImg<iclfloat>()->clear(DC,V)#define SET_8U(V,D,DC) D->asImg<iclbyte>()->clear(DC,V)#define COPY_8U(S,SC,D,DC) memcpy(D->getDataPtr(DC),S->getDataPtr(SC),D->getWidth()*D->getHeight()*sizeof(iclbyte))#define COPY_32F(S,SC,D,DC) memcpy(D->getDataPtr(DC),S->getDataPtr(SC),D->getWidth()*D->getHeight()*sizeof(iclfloat))// }}}
#endif
...
void convertFromRGB8(ImgI *poDstImage, ImgI *poSrcImage){
  // {{{ open    iclbyte *pR = poSrcImage->asImg<iclbyte>()->getData(0);    iclbyte *pG = poSrcImage->asImg<iclbyte>()->getData(1);    iclbyte *pB = poSrcImage->asImg<iclbyte>()->getData(2);        if(poDstImage->getDepth() == depth8u){      switch(poDstImage->getFormat()){        case formatGray:{ //no ROI          register iclbyte *poDst = poDstImage->asImg<iclbyte>()->getData(0);          register iclbyte *poDstEnd = poDst+poDstImage->getDim();          while(poDst!=poDstEnd){            *poDst++=((*pR++)+(*pG++)+(*pB++))/3;           }          break;        }        case formatRGB:          COPY_8U(poSrcImage,0,poDstImage,0);          COPY_8U(poSrcImage,1,poDstImage,1);          COPY_8U(poSrcImage,2,poDstImage,2);          break;        case formatHLS:          {            init_table();            register iclbyte *pH = poDstImage->asImg<iclbyte>()->getData(0);            register iclbyte *pL = poDstImage->asImg<iclbyte>()->getData(1);            register iclbyte *pS = poDstImage->asImg<iclbyte>()->getData(2);            register iclbyte *pHEnd = pH+poDstImage->getDim();            while(pH!=pHEnd){              rgb_to_hls(*pR++,*pG++,*pB++,*pH++,*pL++,*pS++);            }            break;          }          break;        case formatYUV:        case formatLAB:        default:          ERROR_LOG("unsupported format!");      }    }else{//depth32f      switch(poDstImage->getFormat()){        case formatGray:{          register iclfloat *poDst = poDstImage->asImg<iclfloat>()->getData(0);          register iclfloat *poDstEnd = poDst+poDstImage->getDim();          while(poDst!=poDstEnd){            *poDst++=(iclfloat)((*pR++)+(*pG++)+(*pB++))/3.0;          }          break;        }        case formatRGB:          CONVERT_8U32F(poSrcImage,0,poDstImage,0);          CONVERT_8U32F(poSrcImage,1,poDstImage,1);          CONVERT_8U32F(poSrcImage,2,poDstImage,2);          break;        case formatHLS:          {            init_table();            register iclfloat *pH = poDstImage->asImg<iclfloat>()->getData(0);            register iclfloat *pL = poDstImage->asImg<iclfloat>()->getData(1);            register iclfloat *pS = poDstImage->asImg<iclfloat>()->getData(2);            register iclfloat *pHEnd = pH+poDstImage->getDim();                        while(pH!=pHEnd){              rgb_to_hls(*pR++,*pG++,*pB++,*pH++,*pS++,*pL++);            }          }          break;        case formatYUV:        case formatLAB:        default:          ERROR_LOG("unsupported format!");      }    }}  // }}}

void convertFromRGB32(ImgI *poDstImage, ImgI *poSrcImage){
  // {{{ open    iclfloat *pR = poSrcImage->asImg<iclfloat>()->getData(0);    iclfloat *pG = poSrcImage->asImg<iclfloat>()->getData(1);    iclfloat *pB = poSrcImage->asImg<iclfloat>()->getData(2);    if(poDstImage->getDepth() == depth8u){          switch(poDstImage->getFormat()){        case formatGray:{ //no ROI          register iclfloat *poDst = poDstImage->asImg<iclfloat>()->getData(0);          register iclfloat *poDstEnd = poDst+poDstImage->getDim();          while(poDst!=poDstEnd){            *poDst++=(iclbyte)(((*pR++)+(*pG++)+(*pB++))/3);          }          break;        }        case formatRGB:          CONVERT_32F8U(poSrcImage,0,poDstImage,0);          CONVERT_32F8U(poSrcImage,1,poDstImage,1);          CONVERT_32F8U(poSrcImage,2,poDstImage,2);          break;        case formatHLS:          {            init_table();            register iclbyte *pH = poDstImage->asImg<iclbyte>()->getData(0);            register iclbyte *pL = poDstImage->asImg<iclbyte>()->getData(1);            register iclbyte *pS = poDstImage->asImg<iclbyte>()->getData(2);            register iclbyte *pHEnd = pH+poDstImage->getDim();            while(pH!=pHEnd){              rgb_to_hls(*pR++,*pG++,*pB++,*pH++,*pL++,*pS++);            }            break;          }        case formatLAB:        case formatYUV:        default:          ERROR_LOG("unsupported format !");      }    }else{      switch(poDstImage->getFormat()){        case formatGray:          {            register iclfloat *poDst = poDstImage->asImg<iclfloat>()->getData(0);            register iclfloat *poDstEnd = poDst+poDstImage->getDim();            while(poDst!=poDstEnd){              *poDst++=((*pR++)+(*pG++)+(*pB++))/3.0;            }            break;          }        case formatRGB:          COPY_32F(poSrcImage,0,poDstImage,0);          COPY_32F(poSrcImage,1,poDstImage,1);          COPY_32F(poSrcImage,2,poDstImage,2);          break;        case formatHLS:          {            init_table();            register iclfloat *pH = poDstImage->asImg<iclfloat>()->getData(0);            register iclfloat *pL = poDstImage->asImg<iclfloat>()->getData(1);            register iclfloat *pS = poDstImage->asImg<iclfloat>()->getData(2);            register iclfloat *pHEnd = pH+poDstImage->getDim();            while(pH!=pHEnd){              rgb_to_hls(*pR++,*pG++,*pB++,*pH++,*pL++,*pS++);            }            break;          }        case formatLAB:        case formatYUV:        default:          ERROR_LOG("unsupported format!");          break;      }    }}  // }}}
...
void convertYUV420ToRGB8(Img8u *poDst, unsigned char *pucSrc,const Size &s){
  // {{{ open
#ifdef WITH_IPP_OPTIMIZATION
  iclbyte *apucSrc[] = {pucSrc,pucSrc+s.getDim(), pucSrc+s.getDim()+s.getDim()/4};
  iclbyte *apucDst[] = {poDst->getData(0),poDst->getData(1),poDst->getData(2)};
