...
// GridNode — geom2 port of geom::GridSceneObject: an nx*ny lattice rendered as
...
using namespace icl;
using namespace icl::geom2;  // brings geom2::Vec (FixedColVector<float,4>)

// Regular lattice: dimensions, indexing, and node positions.
ICL_REGISTER_TEST("geom2.gridnode.regular", "GridNode regular lattice: size + node positions")
{
...
ICL_REGISTER_TEST("geom2.gridnode.mutable", "GridNode getNode is mutable")
{
...
ICL_REGISTER_TEST("geom2.gridnode.points", "GridNode from explicit points + size guard")
{
...
ICL_REGISTER_TEST("geom2.gridnode.deepcopy", "GridNode deepCopy is independent")
{
  GridNode g(2, 2, Vec(0, 0, 0, 1), Vec(1, 0, 0, 0), Vec(0, 1, 0, 0));
