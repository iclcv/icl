...
// geom2 replacement for PointCloudCreator::mapImage, and the engine behind the
...
namespace {
  ...
  void makeScene(Scene2 &scene, const Size &res) {
    Camera cam = Camera::lookAt(geom2::Vec(0, -600, 350, 1), geom2::Vec(0, 0, 40, 1),
                                geom2::Vec(0, 0, 1, 1), res, 42.0f);
...
ICL_REGISTER_TEST("geom2.mapcolor.zero_baseline_matches_aligned",
...
ICL_REGISTER_TEST("geom2.mapcolor.baseline_drops_unseen",
                  "a stereo baseline leaves some depth points uncoloured")
{
  ...
  geom2::Vec h = cam0.getHoriz();
  const float hn = std::sqrt(h[0]*h[0] + h[1]*h[1] + h[2]*h[2]);
  const float b = 250.f;
  colorCam.translate(geom2::Vec(h[0]/hn*b, h[1]/hn*b, h[2]/hn*b, 0));
