...
ICL_REGISTER_TEST("geom2.defaultscene.studio_furniture", "Studio builds a ground, frame and a 4-light rig")
{
...
ICL_REGISTER_TEST("geom2.defaultscene.void_minimal", "Void has lights + frame + camera but no ground")
{
...
ICL_REGISTER_TEST("geom2.defaultscene.preset_switch_no_leak", "preset switch rebuilds without leaking lights/cameras")
{
  DefaultScene s(SceneType::Studio);
  ICL_TEST_EQ(s.getLightCount(), 4);
...
ICL_REGISTER_TEST("geom2.defaultscene.user_content_survives", "rebuild keeps user-added content")
{
  DefaultScene s(SceneType::Studio);
...
ICL_REGISTER_TEST("geom2.defaultscene.ground_toggle", "ground knob toggles visibility, not the scene graph")
{
  DefaultScene s(SceneType::Studio);
...
ICL_REGISTER_TEST("geom2.defaultscene.frame_toggle", "coordinate frame defaults hidden, knob toggles visibility")
{
  DefaultScene s(SceneType::Studio);
...
ICL_REGISTER_TEST("geom2.defaultscene.up_axis_rebuild", "up-axis flip rebuilds cleanly")
{
  DefaultScene s(SceneType::Studio);
  s.setUpAxis('Z');
  ICL_TEST_EQ(s.getLightCount(), 4);
