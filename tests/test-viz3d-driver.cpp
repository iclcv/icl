...
ICL_REGISTER_TEST("viz3d.driver.attach_sets_node", "addDriver wires node() + calls onAttach")
{
...
ICL_REGISTER_TEST("viz3d.driver.typed_lookup", "getDriver<T> finds the attached driver")
{
  auto cube = CuboidNode::createCube(0, 0, 0, 10);
...
ICL_REGISTER_TEST("viz3d.driver.remove_detaches", "removeDriver clears node() + calls onDetach")
{
  auto cube = CuboidNode::createCube(0, 0, 0, 10);
  auto d = cube->addDriver<CountingDriver>();
...
ICL_REGISTER_TEST("viz3d.driver.not_copied", "deep-copied node starts driverless")
{
  auto cube = CuboidNode::createCube(0, 0, 0, 10);
