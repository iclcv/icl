...
namespace {
  ...
  void makeCubeScene(Scene2 &scene, const Size &res) {
    scene.addCamera(Camera::lookAt(geom2::Vec(0, 0, 600, 1),   // eye
                                   geom2::Vec(0, 0, 0, 1),     // center
                                   geom2::Vec(0, 1, 0, 1),     // up
                                   res, 40.0f));
...
ICL_REGISTER_TEST("geom2.scenecapture.bvh_color_and_depth", "BVH capture yields sized color+depth, cube hit at ~500mm")
{
...
ICL_REGISTER_TEST("geom2.scenecapture.bvh_invalid_camera", "BVH capture returns empty on bad camera index")
{
...
ICL_REGISTER_TEST("geom2.scenecapture.depth_modes", "DistToCamCenter >= DistToCamPlane, equal at centre")
{
...
ICL_REGISTER_TEST("geom2.scenecapture.bvh_caching", "cached capture matches uncached; invalidate picks up changes")
{
