...
namespace icl{
  namespace math{
    ...
    */
    ...
    inline ICL_MATH_API_T DynMatrix<T> &matrix_init(DynMatrix<T> &m, Init init){
    ...
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_abs(DynMatrix<T> &m);
  
    /// element-wise absolute value  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_abs(const DynMatrix<T> &m, DynMatrix<T> &dst);
  
    /// element-wise logarith (basis E) (inplace)  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_log(DynMatrix<T> &m);
  
    /// element-wise logarith (basis E)  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_log(const DynMatrix<T> &m, DynMatrix<T> &dst);
  
    /// element-wise exp-function (inplace)  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_exp(DynMatrix<T> &m);
  
    /// element-wise exp-function  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_exp(const DynMatrix<T> &m, DynMatrix<T> &dst);
  
    /// element-wise square-root-function (inplace)  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_sqrt(DynMatrix<T> &m);
  
    /// element-wise square-root-function  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_sqrt(const DynMatrix<T> &m, DynMatrix<T> &dst);
  
    /// element-wise square-function (x*x) (inplace)  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_sqr(DynMatrix<T> &m);
  
    /// element-wise square-function (x*x)  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_sqr(const DynMatrix<T> &m, DynMatrix<T> &dst);
  
    /// element-wise sinus-function (x*x) (inplace) \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_sin(DynMatrix<T> &m);
  
    /// element-wise sinus-function (x*x) \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_sin(const DynMatrix<T> &m, DynMatrix<T> &dst);
  
    /// element-wise cosinus-function (inplace) \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_cos(DynMatrix<T> &m);
  
    /// element-wise cosinus-function \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_cos(const DynMatrix<T> &m, DynMatrix<T> &dst);
  
    /// element-wise tangent-function (inplace) \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_tan(DynMatrix<T> &m);
  
    /// element-wise tangent-function \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_tan(const DynMatrix<T> &m, DynMatrix<T> &dst);
  
    /// element-wise arcus sinus-function (inplace) \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_arcsin(DynMatrix<T> &m);
  
    /// element-wise arcus sinus-function \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_arcsin(const DynMatrix<T> &m, DynMatrix<T> &dst);
  
    /// element-wise arcus cosinus-function (inplace) \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_arccos(DynMatrix<T> &m);
  
    /// element-wise arcus cosinus-function \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_arccos(const DynMatrix<T> &m, DynMatrix<T> &dst);
  
    /// element-wise arcus tangent-function (inplace) \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_arctan(DynMatrix<T> &m);
  
    /// element-wise arcus tangent-function \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_arctan(const DynMatrix<T> &m, DynMatrix<T> &dst);
  
    /// element-wise reciprocal-function (1/x) (inplace) \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_reciprocal(DynMatrix<T> &m);
  
    /// element-wise reciprocal-function (1/x) \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_reciprocal(const DynMatrix<T> &m, DynMatrix<T> &dst);
    ...
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_powc(DynMatrix<T> &m, T exponent);
  
    /// element-wise power-function (x^exponent)
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_powc(const DynMatrix<T> &m, T exponent, DynMatrix<T> &dst);
  
    /// element-wise addition of constant value (inplace)  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_addc(DynMatrix<T> &m, T val);
  
    /// element-wise addition of constant value  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_addc(const DynMatrix<T> &m, T val, DynMatrix<T> &dst);
  
    /// element-wise substraction of constant value (inplace)  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_subc(DynMatrix<T> &m, T val);
  
    /// element-wise substraction of constant value  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_subc(const DynMatrix<T> &m, T val, DynMatrix<T> &dst);
  
    /// element-wise division by constant value (inplace)  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_divc(DynMatrix<T> &m, T  val);
  
    /// element-wise division by constant value  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_divc(const DynMatrix<T> &m, T val, DynMatrix<T> &dst);
  
    /// element-wise multiplication with constant value (inplace)  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_mulc(DynMatrix<T> &m, T val);
  
    /// element-wise multiplication with constant value  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_mulc(const DynMatrix<T> &m, T val, DynMatrix<T> &dst);
    ...
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_arctan2(const DynMatrix<T> &my, const DynMatrix<T> &mx, DynMatrix<T> &dst)
  	throw (IncompatibleMatrixDimensionException);
  
    /// element-wise addition  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_add(const DynMatrix<T> &m1, const DynMatrix<T> &m2, DynMatrix<T> &dst)
  	throw (IncompatibleMatrixDimensionException);
  
    /// element-wise substraction  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_sub(const DynMatrix<T> &m1, const DynMatrix<T> &m2, DynMatrix<T> &dst)
  	throw (IncompatibleMatrixDimensionException);
  
    /// element-wise multiplication  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_mul(const DynMatrix<T> &m1, const DynMatrix<T> &m2, DynMatrix<T> &dst)
  	throw (IncompatibleMatrixDimensionException);
  
    /// element-wise division  [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_div(const DynMatrix<T> &m1, const DynMatrix<T> &m2, DynMatrix<T> &dst)
  	throw (IncompatibleMatrixDimensionException);
  
    /// element-wise power \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_pow(const DynMatrix<T> &m1, const DynMatrix<T> &m2, DynMatrix<T> &dst)
  	throw (IncompatibleMatrixDimensionException);
    ...
    */
    template<class T>
    ICL_MATH_API_T T matrix_distance(const DynMatrix<T> &m1, const DynMatrix<T> &m2, T norm = 2)
  	throw (IncompatibleMatrixDimensionException);
    ...
    */
    template<class T>
    ICL_MATH_API_T T matrix_divergence(const DynMatrix<T> &m1, const DynMatrix<T> &m2)
  	throw (IncompatibleMatrixDimensionException);
    ...
    */
    template<class T>
    ICL_MATH_API_T T matrix_min(const DynMatrix<T> &m, int *x = 0, int *y = 0);
    ...
    */
    template<class T>
    ICL_MATH_API_T T matrix_max(const DynMatrix<T> &m, int *x = 0, int *y = 0);
    ...
    */
    template<class T>
    ICL_MATH_API_T void matrix_minmax(const DynMatrix<T> &m, T dst[2],
    ...
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T T matrix_mean(const DynMatrix<T> &m);
  
    /// calculate matrix variance  [IPP-optimized] \ingroup LINALG
   /** For float and double only */
    template<class T>
    ICL_MATH_API_T T matrix_var(const DynMatrix<T> &m);
    ...
    /** For float and double only
        ...
        */
    template<class T>
    ICL_MATH_API_T T matrix_var(const DynMatrix<T> &m, T mean, bool empiricalMean = true);
    ...
    */
    template<class T>
    ICL_MATH_API_T void matrix_meanvar(const DynMatrix<T> &m, T *mean, T*var);
  
    /// computes matrix standard deviation (sqrt(var)) [IPP-optimized] \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T T matrix_stddev(const DynMatrix<T> &m);
    ...
    */
    template<class T>
    ICL_MATH_API_T T matrix_stddev(const DynMatrix<T> &m, T mean, bool empiricalMean = true);
    ...
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_muladd(const DynMatrix<T> &a, T alpha, const DynMatrix<T> &b, T beta, T gamma, DynMatrix<T> &dst)
  	throw (IncompatibleMatrixDimensionException);
  
    /// computes alpha*a + gamma \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_muladd(const DynMatrix<T> &a, T alpha, T gamma, DynMatrix<T> &dst);
  
    /// applies masking operation (m(i,j) is set to 0 if mask(i,j) is 0) (inplace) \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_mask(const DynMatrix<unsigned char> &mask, DynMatrix<T> &m)
  	throw (IncompatibleMatrixDimensionException);
  
    /// applies masking operation (m(i,j) is set to 0 if mask(i,j) is 0) \ingroup LINALG
    /** For float and double only */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_mask(const DynMatrix<unsigned char> &mask, const DynMatrix<T> &m, DynMatrix<T> &dst)
     	throw (IncompatibleMatrixDimensionException);
    ...
    */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_mult_t(const DynMatrix<T> &src1, const DynMatrix<T> &src2, DynMatrix<T> &dst, int transpDef)
    throw (IncompatibleMatrixDimensionException);
    ...
    */
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &big_matrix_mult_t(const DynMatrix<T> &src1, const DynMatrix<T> &src2, DynMatrix<T> &dst, int transpDef)
    throw (IncompatibleMatrixDimensionException);
  
    /// applies matrix addition on optionally transposed matrices \ingroup LINALG
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_add_t(const DynMatrix<T> &src1, const DynMatrix<T> &src2, DynMatrix<T> &dst, int transpDef)
    throw (IncompatibleMatrixDimensionException);
  
    /// applies matrix substraction on optionally transposed matrices \ingroup LINALG
    template<class T>
    ICL_MATH_API_T DynMatrix<T> &matrix_sub_t(const DynMatrix<T> &src1, const DynMatrix<T> &src2, DynMatrix<T> &dst, int transpDef)
    throw (IncompatibleMatrixDimensionException);
    ...
    */
    template<class T>
    ICL_MATH_API_T void svd_dyn(const DynMatrix<T> &A, DynMatrix<T> &U, DynMatrix<T> &S, DynMatrix<T> &V) throw (utils::ICLException);
    ...
    template<> inline ICL_MATH_API void svd_dyn<icl64f>(const DynMatrix<icl64f> &A, DynMatrix<icl64f> &U, DynMatrix<icl64f> &s, DynMatrix<icl64f> &V) {
      U.setBounds(A.cols(), A.rows());
      V.setBounds(A.cols(), A.cols());
      s.setBounds(1,A.cols());
