...
**                                                                 **
** File   : ICLIO/src/UnicapGrabber.cpp                            **
** Module : ICLIO                                                  **
...
#define DEACTIVATE_PWC_CAM_IN_UNICAP
  ...
  const ImgBase* UnicapGrabberImpl::acquireImage(){
    // {{{ open
    ...
    if(m_poGrabEngine->needsConversion()){
      ...
      m_poConvertEngine->cvtNative(rawData,&m_poImage);
    }else{
